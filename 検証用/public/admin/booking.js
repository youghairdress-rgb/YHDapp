...
import { 
    ...
    query, where, Timestamp, orderBy, getDoc, serverTimestamp
...
const bookingMain = async (auth, user) => {
    ...
    selectedDate.setHours(0, 0, 0, 0); // 時刻をリセット
    let customers = [];
    let menuCategories = [];
    ...
    const listenToReservations = () => {
        ...
        const startOfDay = new Date(selectedDate); // selectedDate is already reset
