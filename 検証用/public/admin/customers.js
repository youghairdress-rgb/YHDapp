...
const customersMain = async (auth, user) => {
        ...
        customers.forEach(customer => {
            ...
            card.innerHTML = `
                ...
                <div class="customer-card-actions">
                    ...
                    <a href="./admin/counseling/index.html?customerId=${customer.id}" class="icon-button" title="AIカウンセリング"><i class="fa-solid fa-wand-magic-sparkles"></i></a>
            ...
            // ★★★ 修正点: 各カードのカメラボタンにイベントリスナーを設定 ★★★
            card.querySelector('.camera-btn').addEventListener('click', (e) => { 
                e.stopPropagation(); 
                editingCustomerId = customer.id; // 写真を撮る対象の顧客IDをセット
                handleTakePhoto(); 
            });
    ...
    onSnapshot(q, (snapshot) => {
        ...
        // 検索欄に値があれば、リアルタイム更新後もフィルタリングを維持
        if (customerSearchInput.value) {
            filterCustomers();
        } else {
            // インデックスが選択されていれば、そのインデックスでフィルタリング
            const activeButton = document.querySelector('.initial-nav button.active');
            if (activeButton) {
                activeButton.click();
            }
        }
