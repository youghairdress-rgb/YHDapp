...
export async function initializeHairSegmenter() {
    ...
    try {
        // Use global MediaPipeVision object (loaded via inline module script in index.html)
        if (!window.MediaPipeVision || !window.MediaPipeVision.ImageSegmenter) {
...
export async function runHairSegmentation(imgElement) {
    ...
    try {
        ...
        logger.log(`Segmentation Image Loaded: ${offscreenImg.naturalWidth}x${offscreenImg.naturalHeight}, Src: ${src.substring(0, 50)}...`);
        ...
        logger.log(`Mask Dimensions: ${maskWidth}x${maskHeight}, Image Dimensions: ${canvas.width}x${canvas.height}`);
