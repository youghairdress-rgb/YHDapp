...
const main = async () => {
    ...
    if (!salonSettings || !salonSettings.startTime || !salonSettings.endTime) {
    ...
    const updateTimetableUI = (reservations) => {
        ...
        const [openH, openM] = salonSettings.startTime.split(':').map(Number);
        const [closeH, closeM] = salonSettings.endTime.split(':').map(Number);
    ...
    const generateAllTimeSlotsForWeek = (datesOfWeek, reservations) => {
        ...
        datesOfWeek.forEach(date => {
            ...
            const isHoliday = (salonSettings.holidays || []).includes(date.getDay()) || (salonSettings.specialHolidays || []).includes(dateStr);
    ...
    const submitBooking = async () => {
        ...
        try {
            ...
            const bookingData = {
                ...
                lineDisplayName: profile.displayName, // ▼▼▼ LINE表示名を追加 ▼▼▼
